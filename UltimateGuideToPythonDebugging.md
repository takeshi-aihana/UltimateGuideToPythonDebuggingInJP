...
自分のコードを少し改善してもっとデバッグしやすくしたいのであれば、自分のクラスに ``__repr_()_`` メソッドを追加してみて下さい。
このメソッドについて馴染みがない？
そんな場合は「クラスのインスタンスを説明する文字列を返す」、これだけです。
この ``__repr__()`` メソッドを使った最もよく見かける例は、インスタンスを再生性するために使用した文字列を出力にするというものです。
例えば：
...
```
もし上の例のようにオブジェクトを「表現」してしまうのが望ましくない、あるいは不可能である場合、```<...>``` を使った表現を使用することをおすすめします。
