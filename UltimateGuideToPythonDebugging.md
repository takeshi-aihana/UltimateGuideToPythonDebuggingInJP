...
何が起こっているか確認する機会が得られる前にアプリケーションがクラッシュしてしまった場合、ここで紹介するトリックは非常に便利です。

``-i`` オプション付きでアプリケーションを実行する（``python3 -i app.py``）と、アプリケーションが終了すると同時にインタラクティブ・シェルモードが開始します。
この時点で変数や関数を調査することができます。

それでも不十分な場合、「大きなハンマー」こと ``pdb``（Python デバッガ）を使用できます。
``pdb`` はそれ自体で記事が書けてしまうほど膨大な機能があります。
そのため、ここでは
